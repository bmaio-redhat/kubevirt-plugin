...
export class TemplateDetailComponent extends BaseComponent {
  ...
  async clickEditBootSourceRef() {
    await this.robustClick(
      this.testId('actions-dropdown-item-edit-boot-source-ref'),
    );
  }
  ...
  async navigateToNetworks() {
    ...
    const networkTable = this.locator('table').or(this.testId('network-row')).or(this.locator('[role="grid"]'));
    await networkTable
      .first()
      .waitFor({ state: 'visible', timeout: TestTimeouts.DEFAULT })
      .catch(() => {
...
export class CreateVmTemplateCatalogComponent extends BaseComponent {
  ...
  private readonly _diskSourceRegistryInput = this.testId('disk-boot-source-container-source-input');
  ...
  async filterByBootSourceAvailable(check = true) {
    ...
    if (isWizardMode) {
      ...
      const bootSourceFilter = this.testId('boot-source-available-Boot source available')
  ...
  async selectDiskSource(
    ...
    switch (diskSource) {
      ...
      case 'Registry':
        ...
        if (username) {
          await this.testId('disk-boot-source-container-source-username').fill(
            username,
          );
        }
        if (password) {
          await this.testId('disk-boot-source-container-source-password').fill(
            password,
          );
        }
        break;
