  ...
  async addDiskInWizard(diskConfig: {
    ...
    if (diskConfig.diskSource) {
      ...
      if (diskConfig.diskSource.value && diskConfig.diskType) {
        const containerSourceInput = this.testId('disk-source-container');
...
export class CreateVmWizardTabsComponent extends BaseComponent {
  ...
  private readonly _wizardAnnotationsEditButton = this.testId('wizard-metadata-annotations-edit')
    .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="annotations"]'));
  ...
  private readonly _wizardDedicatedResourcesEditBtn = this.testId('dedicated-resources-edit');
  private readonly _wizardDeschedulerCheckbox = this.locator('#descheduler');
  private readonly _wizardDeschedulerEditBtn = this.testId('descheduler-edit');

  private readonly _wizardEvictionStrategyCheckbox = this.locator('#eviction-strategy');
  private readonly _wizardEvictionStrategyEditBtn = this.testId('eviction-strategy').locator('button');
  private readonly _wizardHeadlessCheckbox = this.locator('#headless-mode');
  private readonly _wizardHostnameEditBtn = this.testId('wizard-overview-hostname-edit')
    .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="hostname"]'));
  private readonly _wizardHostnameInput = this.locator('input#hostname');
  private readonly _wizardLabelsEditButton = this.testId('wizard-metadata-labels-edit')
    .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="labels"]'));

  private readonly _wizardLabelsInput = this.testId('tags-input');
  ...
  private readonly _wizardNodeSelectorEditButton = this.testId('node-selector-edit')
    .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="node-selector"]'));

  private readonly _wizardSSHKeyEditButton = this.testId('wizard-sshkey-edit');
  private readonly _wizardStartInPauseCheckbox = this.locator('#start-in-pause-mode');
  private readonly _wizardSysprepEditButton = this.testId('wizard-sysprep-edit');
  ...
  async fillWizardOverview(config: {
    ...
    if (config.description) {
      await this.robustClick(
        this.testId('wizard-overview-description-edit')
          .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="description"]')),
      );
      const tabModal = this._tabModal;
    ...
    try {
      ...
      for (const notFoundLocator of notFoundLocators) {
        try {
          ...
          if (isVisible) {
            const text = await notFoundLocator.first().textContent().catch(() => '');
            if (
      ...
      const networksTabContent = this.testId('wizard-tab-Networks')
        .locator('..')
        .locator('..');
...
export class CreateVmWizardComponent extends BaseComponent {
  ...
  private readonly _storageClassSelect = this.testId('storage-class-select');
  private readonly _tabModal = this.locator('#tab-modal');
  private readonly _tagsInput = this.testId('tags-input');
  ...
  async fillWizardMetadata(config: {
    ...
    if (config.labels && config.labels.length > 0) {
      const labelsEditButton = this.testId('wizard-metadata-labels-edit')
        .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="labels"]'));
      await labelsEditButton.waitFor({
        state: 'visible',
        timeout: TestTimeouts.UI_ACTION_COMPLETE,
      });
      await this.robustClick(labelsEditButton);
  ...
  async fillWizardOverview(config: {
    ...
    if (config.hostname) {
      const hostnameEditButton = this.testId('wizard-overview-hostname-edit')
        .or(this.locator('button[data-test^="pw-vm-customize-it-"][data-test*="hostname"]'));
      await hostnameEditButton.waitFor({
        state: 'visible',
        timeout: TestTimeouts.RESOURCE_CREATION,
      });
      await this.robustClick(hostnameEditButton);
