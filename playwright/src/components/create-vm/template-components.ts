...
export class CreateVmTemplatesComponent extends BaseComponent {
  ...
  async filterByBootSourceAvailable(check = true) {
    const bootSourceFilter = this.testId('boot-source-available-Boot source available').locator('input[type="checkbox"]');
    if (check) {
  ...
  async filterByOSName(osName: 'RHEL' | 'Windows' | 'Fedora' | 'CentOS', check = true) {
    const osFilter = this.testId(`osName-${osName}`).locator(
      'input[type="checkbox"]',
    );
  ...
  async filterByWorkload(workload: 'Desktop' | 'Server', check = true) {
    const workloadFilter = this.testId(`workload-${workload}`).locator(
      'input[type="checkbox"]',
    );
...
export class TemplateCustomizeWizardComponent extends BaseComponent {
  ...
  private readonly _cDROMDiskTypeSelectCdrom = this.locator('text=CD-ROM').or(this.testId('disk-type-select-cdrom'));
  ...
  private readonly _wizardCreateButton = this.testId('create-virtual-machine').locator('button', { hasText: 'Create VirtualMachine' });
  ...
  async addCDROMDisk(
    ...
    const saveButton = this.locator('[role="dialog"]').getByTestId('save-button')
    ...
    await dialog.waitFor({ state: 'hidden', timeout: 60000 }).catch(async (e) => {
      ...
      const cancelButton = this.locator('#tab-modal').getByTestId('cancel-button')
        .or(this.locator('#tab-modal button:has-text("Cancel")')).first();
  ...
  async verifyCdromSizeNotConstrainedByRootDisk(): Promise<boolean> {
    ...
    const cancelButton = this.locator('#tab-modal').getByTestId('cancel-button')
      ...
      .or(this.locator('[role="dialog"] button:has-text("Cancel")')).first();
    if (await cancelButton.isVisible()) {
      await cancelButton.click({ force: true });
    }
