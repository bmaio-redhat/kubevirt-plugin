...
export default class NavigationComponent extends BaseComponent {
  ...
  private readonly _perspectiveSwitcherMenuOption = this.testId(
    'perspective-switcher-menu-option',
  );
  private readonly _perspectiveSwitcherToggle = this.testId('perspective-switcher-toggle');
  ...
  protected async clickSidebarNavItem(
    ...
    await navLocator.scrollIntoViewIfNeeded({ timeout: TestTimeouts.UI_DELAY_MEDIUM }).catch(
      () => undefined,
    );
    await this.page.waitForTimeout(TestTimeouts.UI_DELAY_MICRO);

    await navLocator.click({ force: true, timeout: TestTimeouts.UI_DELAY_MEDIUM }).catch(
      async () => {
  ...
  async expandVirtualizationNavSection(): Promise<void> {
    ...
    const childVisible = await childItem
      .first()
  ...
  async switchToPerspective(perspectiveName: string): Promise<void> {
    await this.openPerspectiveDropdown();
    const perspectiveOption = this.testId('perspective-switcher-menu-option').filter({
      has: this.locator('.pf-v6-c-menu__item-text', {
        hasText: new RegExp(`^${perspectiveName}$`, 'i'),
      }),
    });
  ...
  async switchToVirtualizationPerspective(): Promise<void> {
    ...
    const toggle = this.testId('perspective-switcher-toggle');
    ...
    if (!toggleVisible) {
      ...
      for (let i = 0; i < 3; i++) {
        const hamburgerVisible = await navToggle
          .isVisible()
          .catch(() => false);
        if (!hamburgerVisible) {
    ...
    const virtOption = this.testId('perspective-switcher-menu-option').filter({
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (menuOpened) {
        await virtOption
          .locator('button')
          .first()
          .click({ force: true, timeout: TestTimeouts.DEFAULT });
        await this.page.waitForLoadState('domcontentloaded');
        await this.waitForLoadingComplete(TestTimeouts.SHORT_WAIT);
        return;
      }
