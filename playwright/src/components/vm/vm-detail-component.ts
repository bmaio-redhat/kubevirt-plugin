...
export default class VmDetailComponent extends PageCommons {
  ...
  private readonly _cpuMemoryButton = this.testId(
    'virtual-machine-overview-details-cpu-memory',
  );
  ...
  async isVmNameSpanVisible(
    ...
    try {
      const vmNameSpan = this.testId('vms-treeview')
        .locator('span', { hasText: vmName })
        .first();
  ...
  async verifyDetailsCard(
    ...
    try {
      ...
      if (templateName) {
        ...
        if (!templateExists) {
          const byDataTest = this.testId(templateName);
