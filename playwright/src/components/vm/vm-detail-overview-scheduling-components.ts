  ...
  async verifyDisksCard(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const diskRows =
          this._virtualMachinesOverviewTabDisksMain.locator('[data-test^="disk-"]');
...
export class VirtualMachineDetailSchedulingComponent extends BaseComponent {
  private readonly _affinityModalApplyButton = this.locator('[role="dialog"]').getByTestId(
    'save-button',
  );
  private readonly _affinityModalCancelButton = this.locator('[role="dialog"]').getByTestId(
    'cancel-button',
  );
  private readonly _affinityRulesButton = this.locator('[role="tabpanel"]').getByTestId(
    'affinity-rules',
  );
  ...
  private readonly _tabModalSaveButton = this.locator('#tab-modal').getByTestId('save-button');
