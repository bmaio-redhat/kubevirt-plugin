...
const test = baseTest.extend<GatingFixtures>({
  ...
  _autoVirtNavigation: async ({ page }, use) => {
    ...
    const onboardingDismiss = page.getByTestId('onboarding-dismiss-btn');
    if (await onboardingDismiss.isVisible({ timeout: TestTimeouts.RETRY_DELAY }).catch(() => false)) {
    ...
    const perspToggle = page.getByTestId('perspective-switcher-toggle');
    if (
      !(await perspToggle.isVisible({ timeout: TestTimeouts.RETRY_DELAY }).catch(() => false))
    ) {
