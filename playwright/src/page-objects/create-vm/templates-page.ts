...
export default class TemplatesPage extends PageCommons {
  ...
  override async clickTemplateByTestId(templateName: string) {
    ...
    const loadingSpinner = this.locator('.pf-v6-c-spinner, .pf-c-spinner').or(this.testId('loading'));
  ...
  async openFilterDropdown() {
    ...
    if (!isVisible) {
      await this.robustClick(
        this.testId('filter-toolbar').getByTestId('filter-dropdown-toggle'),
      );
  ...
  async selectPvcAsBootSource(projectName: string, pvcName: string) {
    ...
    await this.robustClick(
      this.testId('disk-source-select-persistentVolumeClaim'),
    );
