...
export class EnvVariables {
  ...
  static get clusterUrl(): string {
    ...
    // Console URLs follow the pattern console-openshift-console.apps.<cluster>.<domain>
    // so the API URL is api.<cluster>.<domain>:6443. This handles BYO clusters where
    // CLUSTER_DOMAIN doesn't match the real infrastructure domain.
    const consoleUrl =
    ...
    // The constructed URL will be wrong unless detect_urls() in playwright-runner.sh
    // has already exported CLUSTER_URL.
  ...
  /**
   * Enable the native ClusterJanitor for in-process stale resource and namespace cleanup.
   * A background sweep runs every `CLUSTER_JANITOR_INTERVAL_MS` milliseconds
   * (default: 60 s) and a final blocking sweep runs in global teardown.
   *
   * Enabled by default. Set ENABLE_CLUSTER_JANITOR=0 to opt out.
   */
  /**
  ...
  /**
   * Skip UDN test namespace creation in global setup.
   * Set SKIP_UDN=1 when the cluster does not have UDN support or tier2 networking tests are not needed.
   */
