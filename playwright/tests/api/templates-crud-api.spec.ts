...
test.describe('Template — user template lifecycle API', { tag: ['@api'] }, () => {
  ...
  test('PATCH: update cpu.cores via JSON Patch (mirrors CNV-82385 Edit CPU)', async ({
    testNamespace,
    apiClient,
  }) => {
  ...
  test('PATCH: update memory via JSON Patch (mirrors CNV-82385 Edit memory)', async ({
    testNamespace,
    apiClient,
  }) => {
    await apiClient.patchTemplate(testNamespace, templateName, [
  ...
  test('DELETE: remove template and confirm it leaves the list (mirrors CNV-8882)', async ({
    testNamespace,
    apiClient,
  }) => {
...
test.describe('Template — dedicated resources API', { tag: ['@api'] }, () => {
  ...
  test('READ: template has highperformance workload label (mirrors CNV-8872)', async ({
    testNamespace,
    apiClient,
  }) => {
    const tmpl = await apiClient.getTemplate(testNamespace, templateName);
  ...
  test('READ: nested VM has dedicatedCpuPlacement=true (mirrors CNV-8874)', async ({
    testNamespace,
    apiClient,
  }) => {
    const tmpl = await apiClient.getTemplate(testNamespace, templateName);
