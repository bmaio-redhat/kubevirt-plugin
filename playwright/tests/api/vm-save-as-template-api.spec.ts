...
test.describe('VM save-as-template — spec parity API', { tag: ['@api'] }, () => {
  ...
  test.beforeAll(async ({ testNamespace, apiClient, k8sClient, utils }) => {
    try {
      ...
      await test.step('GET VM spec + BUILD template + POST template (mirrors CNV-82187)', async () => {
  ...
  test('READ: template CPU cores match source VM (mirrors CNV-81591)', async ({
    testNamespace,
    apiClient,
  }) => {
  ...
  test('READ: template memory matches source VM (mirrors CNV-81591)', async ({
    testNamespace,
    apiClient,
  }) => {
    const [sourceVm, tmpl] = await Promise.all([
  ...
  test('READ: template disk list matches source VM volumes (mirrors CNV-81591)', async ({
    testNamespace,
    apiClient,
  }) => {
    const [sourceVm, tmpl] = await Promise.all([
    ...
    for (const diskName of sourceDisks) {
      expect(tmplDisks, `disk "${diskName}" must appear in template`).toContain(diskName);
    }
  });
