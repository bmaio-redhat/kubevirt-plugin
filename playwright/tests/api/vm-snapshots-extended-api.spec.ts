...
import { pollUntilRestoreComplete, pollUntilSnapshotReady } from '@/utils/api-poll';
...
test.describe('VirtualMachineSnapshot — full lifecycle API', { tag: ['@api'] }, () => {
  ...
  test('CREATE restore: restore VM from snapshot (mirrors CNV-77401)', async ({
    testNamespace,
    apiClient,
  }) => {
    const spec = {
  ...
  test('DELETE restore: remove restore and confirm it leaves the list', async ({
    ...
    await k8sClient.waitForRestoreDeleted(restoreName, testNamespace);
  ...
  test('DELETE snapshot: remove snapshot and confirm it leaves the list (mirrors CNV-8842)', async ({
    testNamespace,
    apiClient,
    k8sClient,
  }) => {
    ...
    await k8sClient.waitForSnapshotDeleted(snapshotName, testNamespace);
...
test.describe('VirtualMachineSnapshot — metadata API', { tag: ['@api'] }, () => {
  ...
  test.beforeAll(async ({ testNamespace, apiClient, k8sClient, utils }) => {
    try {
      ...
      await test.step('WAIT: VM exists', async () => {
        await k8sClient.waitForVmExists(vmName, testNamespace);
      });
  ...
  test('READ: snapshot spec references correct VM (mirrors CNV-8844)', async ({
    testNamespace,
    apiClient,
  }) => {
    const snap = await apiClient.getVirtualMachineSnapshot(testNamespace, snapshotName);
