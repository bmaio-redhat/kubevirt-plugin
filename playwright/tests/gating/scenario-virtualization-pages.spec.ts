...
/** Checks if Storage Migration Plans feature is available via API group discovery. */
async function isStorageMigrationAvailable(k8sClient: KubernetesClient): Promise<boolean> {
  try {
    const result = await (
      k8sClient as unknown as {
        listCustomResources: (...a: unknown[]) => Promise<{ items?: unknown[] }>;
      }
    ).listCustomResources(
...
async function navigateToProjectVmListForNamespace(
  ...
  await vmTreePage.page.waitForTimeout(2_000);
}
...
test.describe('Virtualization pages (gating)', { tag: [GATING_TAG] }, () => {
  ...
  test('Bootable volumes name filter narrows results and clears correctly', async ({
    bootableVolumesPage,
    pageCommons,
    utils,
  }) => {
    ...
    await test.step('Filter by "fedora" shows only matching results', async () => {
      await pageCommons.filterByName(utils.INSTANCE_TYPES.FEDORA);
      // BV table rows use PF6 DataView (no data-test-rows="resource-row"), use the page's own row check
      ...
      expect
        ...
        .toBe(true);
      const otherRow = bootableVolumesPage.page.locator(
        `tr:has-text("${utils.INSTANCE_TYPES.CENTOS_STREAM9}")`,
      );
      const otherHidden = await otherRow
        .waitFor({ state: 'hidden', timeout: utils.TestTimeouts.DEFAULT })
        .then(() => true)
        .catch(() => false);
      expect
        .soft(
    ...
    await test.step('Clearing the name filter restores all bootable volumes', async () => {
      await pageCommons.filterByName('');
      const centosVisible = await bootableVolumesPage.verifyBootableVolumeExistsInList(
      ...
      expect
        .soft(
          centosVisible,
          'CentOS Stream 9 bootable volume should be visible again after clearing the name filter',
        )
        .toBe(true);
      const fedoraStillVisible = await bootableVolumesPage.verifyBootableVolumeExistsInList(
        utils.INSTANCE_TYPES.FEDORA,
        utils.TestTimeouts.DEFAULT,
      );
      expect
        .soft(
          fedoraStillVisible,
          'Fedora bootable volume should still be visible after clearing the filter',
        )
        .toBe(true);
    });
  });
  ...
  test('Storage MigrationPlans page loads with expected columns and content', async ({
    ...
    const storageMigrationAvailable = await isStorageMigrationAvailable(k8sClient);
    test.skip(
    ...
    const hasRows = (await vmListPage.page.locator('tbody tr').count()) > 0;
    if (hasRows) {
  ...
  test('TreeView nodes render distinguishable icons for cluster and namespace types', async ({
    ...
    // Wait for the tree to fully render before inspecting icons
    await vmListPage.page.waitForTimeout(utils.TestTimeouts.UI_ANIMATION_DELAY);
  ...
  test(
    ...
    async ({ overviewPage, pageCommons, utils }) => {
      ...
      const hasFleetVirtualization = await pageCommons.isPerspectiveOptionVisible(
  ...
  test('Empty namespace VM list and overview tabs show correct empty state content', async ({
    ...
    await navigateToProjectVmListForNamespace(vmTreePage, emptyNs);
    ...
    await test.step('VM list tab shows "No virtual machines yet" empty state', async () => {
      ...
      expect
        .soft(bodyText.toLowerCase(), 'Empty state body should mention creating a VirtualMachine')
  ...
  test('VM list empty state project creation hint respects user project creation permission', async ({
    ...
    await navigateToVirtualMachinesWithEmptyProjectsInTree(vmTreePage, testConfig.testNamespace);
    await navigateToProjectVmListForNamespace(vmTreePage, permNs);
  ...
  test('VM list page loads and Virtual machines tab is navigable', async ({
    ...
    await test.step('Virtual machines tab is present and navigable', async () => {
      ...
      // In an empty namespace the page renders an empty state (no table body) — verify
      // the page loaded by checking for either a table or the empty state message
      const pageReady = await vmListPage.page
  ...
  test('VM namespace Overview and Virtual machines tabs switch correctly', async ({
    ...
    await test.step('Overview tab renders content without a JS error boundary', async () => {
      // The page should render either the summary widget (populated namespace) or
      // the empty-state area — either way, no crash boundary should be present.
      const crashed = await vmListPage.page
        .locator(
  ...
  () => {
    test('Scripts and Parameters tabs load for a Red Hat template', async ({
      ...
      await test.step('Parameters tab is active and loaded', async () => {
        ...
        const paramTab = templateDetailPage.page.locator(
          '[data-test-id="horizontal-link-Parameters"][aria-selected="true"], [data-test-id="horizontal-link-Parameters"].pf-m-current',
        );
        const active = await paramTab
          .first()
          .waitFor({ state: 'visible', timeout: utils.TestTimeouts.UI_ELEMENT_VISIBILITY })
          .then(() => true)
          .catch(async () => {
            const tab = templateDetailPage.page.locator(
              '[data-test-id="horizontal-link-Parameters"]',
            );
            return tab
              .waitFor({ state: 'visible', timeout: utils.TestTimeouts.UI_ELEMENT_VISIBILITY })
              .then(() => true)
