...
test.describe(
  ...
  () => {
    test('Wizard boot source step shows description hint and volume table', async ({
      ...
      await utils.withAllure({ suite: 'VM Creation Wizard', feature: GATING, tags: [GATING_TAG] });
      ...
      await test.step('Boot source step description hint is present', async () => {
        ...
        expect(
          descVisible,
          'Boot source step description ("Select a boot source…") should be visible (CNV-84916 fix: hint was missing from the boot source step)',
        ).toBe(true);
      });
...
test.describe('VM Creation Wizard (gating)', { tag: [GATING_TAG, '@catalog-wizard'] }, () => {
  ...
  test('New VM wizard allows full step navigation with expected UI on each step', async ({
    ...
    await test.step('Wizard header has no close (X) button', async () => {
      ...
      expect(closeHidden, 'Wizard header close button should be hidden (CNV-83743)').toBe(true);
    });
    ...
    const customizationVisible = await vmCreationWizardPage.verifyCustomizationStepVisible();
    expect.soft(customizationVisible, 'Customization heading should be visible').toBe(true);

    const customizationTabsVisible = await vmCreationWizardPage.verifyCustomizationTabsVisible();
    expect.soft(customizationTabsVisible, 'All customization tabs should be visible').toBe(true);

    const searchVisible = await vmCreationWizardPage.verifyCustomizationSearchInputVisible();
    expect.soft(searchVisible, '"Find settings" search input should be visible').toBe(true);

    const detailFields = await vmCreationWizardPage.verifyCustomizationDetailsFields();
    expect
    ...
    const expandSections = await vmCreationWizardPage.verifyCustomizationExpandableSections();
    expect
  ...
  test('Tree view namespace right-click "Create VirtualMachine" opens the creation wizard', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'tree-view', 'wizard'],
    });
  ...
  test('Wizard namespace stays in sync with tree view selection and header shows no project selector', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard', 'namespace-sync'],
    });
    ...
    await test.step('Verify no header project selector on wizard page', async () => {
      ...
      expect(
        noHeaderSelector,
        'Header project selector should not be present — removed by CNV-84499 fix',
      ).toBe(true);
    });
  ...
  test('Selecting a template in the wizard enables Next and Review and create navigation buttons', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'template-selection'],
    });
  ...
  test('New VM wizard Next button is disabled until required step data is provided', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard-validation'],
    });
    ...
    await test.step('Guest OS step — OS tiles and type dropdown visible', async () => {
      const osTilesVisible = await vmCreationWizardPage.verifyOsTilesVisible();
      expect(osTilesVisible, 'OS tiles should be visible on Guest OS step').toBe(true);
      const dropdownVisible = await vmCreationWizardPage.verifyOsTypeDropdownVisible();
      expect(dropdownVisible, 'OS type dropdown should be visible').toBe(true);
    });
  ...
  test('From Template wizard Next button is disabled until a template is selected', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard-validation', 'template-flow'],
    });
  ...
  test('Clone VM wizard Next button is disabled until a source VM is selected', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard-validation', 'clone-flow'],
    });
  ...
  test('Guest OS selection is reset to default when the creation wizard is reopened', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard-state-reset'],
    });
    ...
    await test.step('Guest OS reset — previous selection cleared', async () => {
      const currentOsType = await vmCreationWizardPage.getSelectedOsType();
      expect(
        currentOsType,
        'Previously selected OS type (fedora) must be cleared when wizard is reopened — should show default',
      ).not.toBe('fedora');
    });
  ...
  test('Boot source step shows Add volume and No boot source options for OS with no available volumes', async ({
    ...
    await utils.withAllure({ suite: SUITE, feature: GATING, tags: [GATING_TAG, 'boot-source'] });
  ...
  test('VM name field validates RFC 1123 format and Review and Create step does not require Tab key', async ({
    ...
    await utils.withAllure({
      suite: SUITE,
      feature: GATING,
      tags: [GATING_TAG, 'wizard-validation', 'name-validation'],
    });
