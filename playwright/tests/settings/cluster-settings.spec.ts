...
test.describe('Cluster Settings', { tag: [CNV_SETTINGS_TAG, '@adminOnly'] }, () => {
  ...
  test('Memory density toggle and percentage can be set and verified via API', async ({
    settingsPage,
    k8sClient,
    utils,
  }) => {
    ...
    await settingsPage.navigateToGeneralSettings();
    await settingsPage.openMemoryDensitySettings();

    const initiallyEnabled = await settingsPage.getMemoryDensityToggleState();

    if (initiallyEnabled) {
      const disabled = await settingsPage.disableMemoryDensity();
      expect.soft(disabled, 'Memory density disabled from initial enabled state').toBe(true);

      const afterDisable = await k8sClient.verifyMemoryDensity(
        utils.MEMORY_DENSITY.DISABLED_PERCENTAGE,
      );
      expect
        .soft(
          afterDisable.memoryOvercommitPercentage,
          `Memory overcommit should be ${utils.MEMORY_DENSITY.DISABLED_PERCENTAGE} after disable`,
        )
        .toBe(true);

      const reEnabled = await settingsPage.enableMemoryDensity();
      expect.soft(reEnabled, 'Memory density re-enabled').toBe(true);
    } else {
      const enabled = await settingsPage.enableMemoryDensity();
      expect.soft(enabled, 'Memory density enabled from initial disabled state').toBe(true);
    }

    const percentageSet = await settingsPage.setMemoryDensityPercentage(
      utils.MEMORY_DENSITY.ENABLED_PERCENTAGE,
    );
    expect.soft(percentageSet, 'Memory density percentage set').toBe(true);

    const enabledVerification = await k8sClient.verifyMemoryDensity(
      utils.MEMORY_DENSITY.ENABLED_PERCENTAGE,
    );
    expect
      .soft(
        enabledVerification.memoryOvercommitPercentage,
        `Memory overcommit should be ${utils.MEMORY_DENSITY.ENABLED_PERCENTAGE}`,
      )
      .toBe(true);

    const finalDisable = await settingsPage.disableMemoryDensity();
    expect.soft(finalDisable, 'Memory density disabled at end').toBe(true);

    const finalVerification = await k8sClient.verifyMemoryDensity(
      utils.MEMORY_DENSITY.DISABLED_PERCENTAGE,
    );
    expect
      .soft(
        finalVerification.memoryOvercommitPercentage,
        `Memory overcommit should be ${utils.MEMORY_DENSITY.DISABLED_PERCENTAGE} at end`,
      )
      .toBe(true);
  });
