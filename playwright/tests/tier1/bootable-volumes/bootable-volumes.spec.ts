...
async function setupPwNamespace(
  ...
  const ns = utils.generateTestNamespace(prefix);
...
async function createDataVolumeWithYamlViaUi(
  ...
  // Strip the namespace line from the YAML — the bootable volumes editor uses the URL's
  // namespace context. Including a mismatched namespace causes a 422 error.
...
test.describe('Tier1 Virtualization Bootable Volumes Page Tests', { tag: [T1_TAG] }, () => {
  ...
  test(
    ...
    async ({ bootableVolumesPage, k8sClient, utils }) => {
      ...
      const ns = await setupPwNamespace(k8sClient, 'bv-arch-annotation', utils);
