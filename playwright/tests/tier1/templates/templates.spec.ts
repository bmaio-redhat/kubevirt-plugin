...
async function verifyTemplateDeletedFromCluster(
  k8sClient: KubernetesClient,
  templateName: string,
  namespace: string,
  utils: TestUtilsType,
...
test.describe.serial('Tier1 Template Tests', { tag: [T1_TAG, '@tier1-templates'] }, () => {
  ...
  test.beforeAll(async ({ k8sClient, utils }) => {
    try {
      sharedNs = await setupPwNamespace(k8sClient, 'templates-shared', utils);
    } catch (error: unknown) {
...
test.describe.serial(SUITE_TEMPLATE_LIFECYCLE, { tag: [T1_TAG, '@tier1-templates'] }, () => {
  ...
  test.beforeAll(async ({ k8sClient, utils }) => {
    try {
      sharedNs = await setupPwNamespace(k8sClient, 'tpl-lifecycle-shared', utils);
    } catch (error: unknown) {
      setupError = error instanceof Error ? error.message : String(error);
    }
  });
