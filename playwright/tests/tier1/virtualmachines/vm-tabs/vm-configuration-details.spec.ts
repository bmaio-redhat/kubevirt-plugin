...
test.describe.serial('VM Configuration — stopped RHEL9 VM', { tag: [T1_TAG, '@nonpriv'] }, () => {
  ...
  test('Stopped VM configuration CRUD', async ({ apiClient, vmTreePage, vmDetailPage, utils }) => {
    ...
    await test.step('Verify scheduling, eviction, headless, and boot mode sub-tabs', async () => {
      ...
      await expect
        .poll(
          async () => vmDetailPage.verifySchedulingAndResourceRequirements(),
          { timeout: 30_000, intervals: [3_000], message: 'Scheduling sub-tab should load' },
        )
        .toBe(true);

      await expect
        .poll(
          async () => vmDetailPage.verifyEvictionStrategyLiveMigrate(),
          {
            timeout: 30_000,
            intervals: [3_000],
            message: 'Default RHEL9 template should show LiveMigrate eviction strategy',
          },
        )
        .toBe(true);
