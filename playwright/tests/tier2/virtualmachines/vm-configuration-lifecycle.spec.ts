...
test.describe('VM Configuration — startInPause and CPU/memory lifecycle', { tag: [T2_TAG] }, () => {
  test(
    ...
    async ({ k8sClient, vmTreePage, vmDetailPage, pageCommons, utils, testConfig }) => {
      ...
      const headlessVerified = await vmDetailPage.verifyHeadlessChecked(false);
      expect.soft(headlessVerified, 'VM headless mode should be disabled (restored)').toBe(true);
